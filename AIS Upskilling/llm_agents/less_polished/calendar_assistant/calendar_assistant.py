...
GLOBAL_MODEL = "gpt-4.1-mini" # "o4-mini" # "gpt-4.1"

# Global variable to track active reminders
active_reminders = []

def format_time(elapsed_seconds: float) -> str:
    """Format elapsed time in a human-readable format."""
    if elapsed_seconds < 60:
        return f"{elapsed_seconds:.0f} sec"
...
def get_response(conversation, conversation_start_time=None):
    # Log what the assistant sees as input
    if conversation_start_time is not None:
        write_assistant_input(conversation, "get_response", conversation_start_time)
...
functions = [
    ...
    },
    {
        ...
        "function": {
            "name": "set_reminder",
            "description": "Set a reminder for a specific time. Supports two modes: 1) 'from_now': time_seconds is relative to current moment (e.g., 60 = 1 minute from now), 2) 'from_start': time_seconds is absolute time since conversation started (e.g., 120 = when conversation reaches 2 minutes total). Default is 'from_start' for backward compatibility.",
            "parameters": {
                "type": "object",
                "properties": {
                    "time_seconds": {
                        "type": "number",
                        "description": "Time in seconds. If mode='from_now', this is seconds from current moment. If mode='from_start', this is seconds since conversation began."
                    },
                    "message": {
                        "type": "string",
                        "description": "The reminder message to display to your future self. Often, this should be an instruction to yourself to do something."
                    },
                    "mode": {
                        "type": "string",
                        "enum": ["from_now", "from_start"],
                        "description": "Time reference mode: 'from_now' = time_seconds from current moment, 'from_start' = time_seconds since conversation started. Defaults to 'from_start'."
                    }
                },
...
def set_reminder(time_seconds: float, message: str, mode: str = "from_start", conversation_start_time: float = None) -> Dict:
    """Set a reminder for a specific time from conversation start or from now."""
    try:
        current_time = time.time()
        
        if mode == "from_now":
            # Calculate absolute trigger time from current moment
            if conversation_start_time is None:
                return {"error": "conversation_start_time required for 'from_now' mode"}
            current_elapsed = current_time - conversation_start_time
            trigger_time = current_elapsed + time_seconds
            time_description = f"{format_time(time_seconds)} from now (at {format_time(trigger_time)} total)"
        else:  # mode == "from_start" (default)
            trigger_time = time_seconds
            time_description = f"{format_time(time_seconds)} from conversation start"
        ...
        return {
            "status": "success", 
            "message": f"Reminder set for {time_description}: {message}"
        }
    except Exception as e:
        return {"error": str(e)}

def check_and_trigger_reminders(conversation_start_time: float, conversation: List[Dict]) -> List[Dict]:
    """Check for expired reminders and add system messages for them."""
    global active_reminders
    current_time = time.time()
    elapsed_time = current_time - conversation_start_time
    
    triggered_reminders = []
    remaining_reminders = []
    
    for reminder in active_reminders:
        if elapsed_time >= reminder["trigger_time"]:
            # Create system message for triggered reminder
            system_message = {
                "role": "system",
                "content": f"(Time {format_time(reminder['trigger_time'])} reached, it has now been {format_time(elapsed_time)} since the conversation started.) Reminder: {reminder['message']}"
            }
            triggered_reminders.append(system_message)
        else:
            remaining_reminders.append(reminder)
    
    # Update active reminders to only include non-triggered ones
    active_reminders = remaining_reminders
    
    return triggered_reminders

def handle_tool_call(tool_call, conversation_start_time=None):
    ...
    if tool_call.function.name == "list_files":
        ...
        mode = args.get("mode", "from_start")  # Default to from_start for backward compatibility
        result = set_reminder(args["time_seconds"], args["message"], mode, conversation_start_time)
    else:
...
def write_assistant_input(conversation, function_name, conversation_start_time):
    """Write what the assistant sees as input to what_the_assistant_saw.txt"""
    elapsed_time = time.time() - conversation_start_time
    
    data = {
        "function": function_name,
        "time_since_start": format_time(elapsed_time),
        "elapsed_seconds": elapsed_time,
        "conversation": conversation
    }
    
    # Append to file to track all API calls
    mode = "a" if os.path.exists("what_the_assistant_saw.txt") else "w"
    with open("what_the_assistant_saw.txt", mode, encoding='utf-8') as f:
...
def get_structured_response(conversation, conversation_start_time=None):
    ...
    # Log what the assistant sees as input
    if conversation_start_time is not None:
        write_assistant_input(conv_with_question, "get_structured_response", conversation_start_time)
...
def main():
    ...
    while user_message.lower() != 'end':
        ...
        # Check for triggered reminders before processing user message
        triggered_reminders = check_and_trigger_reminders(conversation_start_time, conversation)
        for reminder_msg in triggered_reminders:
            conversation.append(reminder_msg)
            print(f"System: {reminder_msg['content']}")
            write_transcript(conversation)
        
        # Calculate elapsed time and prepend to user message
        elapsed_time = time.time() - conversation_start_time
        time_prefix = f"[Time since start of conversation: {format_time(elapsed_time)}] "
        formatted_user_message = time_prefix + user_message
            
        conversation.append({"role": "user", "content": formatted_user_message})
        ...
        while not assistant_done and loop_count < 10:
            response = get_response(conversation, conversation_start_time)
            ...
            if response.tool_calls:
                ...
                tool_outputs = []
                for tool_call in response.tool_calls:
                    # Check for triggered reminders before each tool call
                    triggered_reminders = check_and_trigger_reminders(conversation_start_time, conversation)
                    for reminder_msg in triggered_reminders:
                        conversation.append(reminder_msg)
                        print(f"System: {reminder_msg['content']}")
                    ...
                    tool_result = handle_tool_call(tool_call, conversation_start_time)
                ...
                follow_up_response = get_response(conversation, conversation_start_time)
            ...
            # print("DEBUG: About to call get_structured_response")
            # pause = input("Enter to continue")
            structured_response = get_structured_response(conversation, conversation_start_time)
            ...
            if structured_response['continue_option'] in continue_options:
                # Check for triggered reminders only when continuing
                triggered_reminders = check_and_trigger_reminders(conversation_start_time, conversation)
                for reminder_msg in triggered_reminders:
                    conversation.append(reminder_msg)
                    print(f"System: {reminder_msg['content']}")
