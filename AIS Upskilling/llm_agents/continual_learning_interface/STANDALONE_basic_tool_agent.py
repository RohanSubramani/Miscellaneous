...
GLOBAL_MODEL = "gpt-4.1-mini" # "o4-mini" # "gpt-5"
...
def handle_tool_call(tool_call, web_mode=False, script_confirmation=None):
    """
    Handle a tool call.
...
def get_structured_response(conversation, web_mode=False):
...
def process_user_message(conversation, user_message, web_mode=False, script_confirmation=None):
    """
    ...
    Args:
        ...
        web_mode: If True, operate in web mode (no console I/O)
        script_confirmation: For run_python_script, 'approved' or 'cancelled'
    
    Returns:
        dict with:
            - steps: List of step objects with type, content, tool_calls, etc.
            - conversation: Updated conversation
            - needs_confirmation: If True, needs user confirmation for a tool call
            - confirmation_tool_call: The tool call that needs confirmation
    """
    ...
    while not assistant_done and loop_count < 10:
        ...
        if response.tool_calls:
            ...
            for tool_call in response.tool_calls:
                tool_result = handle_tool_call(tool_call, web_mode=web_mode, script_confirmation=script_confirmation)
            ...
            # Record assistant response step
            steps.append({
...
def main():
    ...
    while user_message.lower() != 'end':
        result = process_user_message(conversation, user_message, web_mode=False)
