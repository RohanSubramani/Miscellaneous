...
def internal_review(conversation):
    ...
    system_prompt = (
        ...
        "The user will provide you with their conversation history with the assistant so far. "
        ...
        "You should ONLY reject a tool call if you think the user might be upset if the assistant took that action. "
    ...
    supervisor_reply = response.choices[0].message.parsed
    explanation = supervisor_reply.explanation
    option = supervisor_reply.option
    ...
    if option in [2, 3]:
        ...
        cost_explanation = (
            ...
            "Terminating prevents the task from being completed, escalation requires the user to look which can be annoying. "
            "Please confirm that you still want to proceed with your decision, considering the cost."
        )
        ...
        confirmation_response = client.beta.chat.completions.parse(
            model=global_model,
            messages=supervisor_messages,
            response_format=ReasonAndSelect,
        )
        confirmation_reply = confirmation_response.choices[0].message.parsed
        confirmation_explanation = confirmation_reply.explanation
        confirmation_option = confirmation_reply.option
